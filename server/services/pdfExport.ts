...
// Export result types for other formats
...
/**
 * Generate Markdown export from insight content
 */
export async function generateMarkdownExport(
  ...
  let markdown = `# ${title}\n\n`;
  markdown += `*Insights from "${bookTitle}"${author ? ` by ${author}` : ""}*\n\n`;
  markdown += `Generated on ${generatedAt.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}\n\n`;
  
  if (keyThemes.length > 0) {
    markdown += `**Key Themes:** ${keyThemes.join(", ")}\n\n`;
  }
  ...
  for (const section of sections) {
    markdown += renderSectionToMarkdown(section);
  }
  
  markdown += `\n---\n\n*Generated by Insight Atlas*\n`;
  
  const markdownKey = `insights/${insightId}/export-${Date.now()}.md`;
  const { url: markdownUrl } = await storagePut(markdownKey, Buffer.from(markdown), "text/markdown");
...
/**
 * Generate Plain Text export from insight content
 */
export async function generatePlainTextExport(
  options: PDFExportOptions,
  insightId: number
): Promise<PlainTextExportResult> {
  const { title, author, summary, sections, keyThemes, bookTitle, generatedAt } = options;
  
  let text = `${title.toUpperCase()}\n`;
  text += `${"=".repeat(title.length)}\n\n`;
  text += `Insights from "${bookTitle}"${author ? ` by ${author}` : ""}\n`;
  text += `Generated on ${generatedAt.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}\n\n`;
  ...
  for (const section of sections) {
    text += renderSectionToPlainText(section);
  }
  
  text += `\n${"─".repeat(50)}\n\nGenerated by Insight Atlas\n`;
  
  const textKey = `insights/${insightId}/export-${Date.now()}.txt`;
  const { url: textUrl } = await storagePut(textKey, Buffer.from(text), "text/plain");
...
/**
 * Generate HTML export from insight content
 */
...
/**
 * Render a section to Markdown format
 */
function renderSectionToMarkdown(section: InsightSection): string {
  switch (section.type) {
    case "heading":
    ...
    case "authorSpotlight":
      return `**Author Spotlight:** ${section.content}\n\n`;
      
    case "insightNote":
      return `💡 **${section.title || "Key Insight"}:** ${section.content}\n\n`;
      
    case "alternativePerspective":
      return `🔄 **Alternative Perspective:** ${section.content}\n\n`;
      
    case "researchInsight":
      return `📊 **Research Insight:** ${section.content}\n\n`;
      
    case "keyTakeaways":
      const takeaways = section.items?.map(item => `- ✓ ${item}`).join("\n") || "";
      return `### Key Takeaways\n\n${takeaways}\n\n`;
      
    case "exercise":
      return `✏️ **${section.title || "Exercise"}:** ${section.content}\n\n`;
    ...
    case "numberedList":
      const numbered = section.items?.map((item, i) => `${i + 1}. ${item}`).join("\n") || "";
      return `${section.title ? `### ${section.title}\n\n` : ""}${numbered}\n\n`;
...
/**
 * Render a section to Plain Text format
 */
function renderSectionToPlainText(section: InsightSection): string {
  switch (section.type) {
    case "heading":
    ...
    case "authorSpotlight":
      return `[Author Spotlight]\n${section.content}\n\n`;
      
    case "insightNote":
      return `[${section.title || "Key Insight"}]\n${section.content}\n\n`;
